...
public sealed class DicomStudyGenerator
{
    ...
    {
        _cfg = cfg; _rnd = rnd; _factory = new DicomFactory(cfg);
    }
    ...
    private static List<int> SplitPositive(int total, int parts, Random rnd)
    {
        ...
        var points = new List<int> { 0 }; points.AddRange(cuts); points.Add(total);
        var outp = new List<int>();
        for (int i=1;i<points.Count;i++) outp.Add(points[i]-points[i-1]);
        return outp;
    }

    public IEnumerable<string> GenerateStudy(
    string outputRoot,
    string organization,
    string patientId,
    string patientName,
    ...
    DateTime baseDateUtc)
    {
        ...
        var studyUid = FellowOakDicom.DicomUIDGenerator.GenerateDerivedFromUUID().UID;
        ...
        for (int s = 0; s < perSeries.Count; s++)
        {
            ...
            // Combined Modality + Series description folder
            var combinedSeriesFolder = Sanitizer.ForPath($"{modality} {seriesDesc}");
            var seriesUid = FellowOakDicom.DicomUIDGenerator.GenerateDerivedFromUUID().UID;
            ...
            for (int i = 0; i < perSeries[s]; i++)
            {
                ...
                _factory.WriteInstance(path, patientId, patientName, modality, studyDescription, baseDateUtc,
